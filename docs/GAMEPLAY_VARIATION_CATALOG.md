...
- Math mixed-operation direct-result -> existing `make_total` / `take_away`: audit PR #237 -> main `c01d0bac`; CI #1086/#1087 full success including exact audit-main Cloudflare smoke. Runtime implementation active for exact 4 IDs with hardened exact 7+7 families and dedicated add/sub browser QA; expected `choice_grid` 179 / `make_total` 7 / `take_away` 7 / 47 active; `math-mixed-choose-add` excluded; **RUNTIME ACTIVE / NO PATTERN #48**.
