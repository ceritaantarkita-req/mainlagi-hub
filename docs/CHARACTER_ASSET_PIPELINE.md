...
The legacy `/artwork/garden-paca.webp` and `/artwork/garden-gavi.webp` files were removed in Session 15 after Session 14 had already removed all known normal direct product-surface consumption. The old legacy runtime registry/API path was removed at the same time. Character runtime is now SVG-only.
