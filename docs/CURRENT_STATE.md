...
## 25 September character + World safe checkpoint — LIVE VERIFIED THROUGH SESSION 09

```text
Session 09 runtime implementation main: 227a77799cd73fecc8e58960ef8758c2e323bc30
Session 09 PR: #340
final PR head after main sync: 80e2a7211e6ebe1791370a87790b4c2ff20a621a
final PR CI: #1673 / run 36131389534 — full success
runtime merged-main CI: #1674 / run 36132350678 — full success
runtime Production smoke (Cloudflare): success
Sessions 01–09: COMPLETE
next: Session 10 — semantic/activity illustration registry SVG-aware production contract
```

The exact repository `main` HEAD may move through later docs-only descendants. Treat the SHAs above as the verified Session 09 runtime checkpoint, and query `main` itself when an exact latest repository HEAD is required.

Session 08 migrated Petualangan Uang to the shared approved SVG character runtime while preserving the authored Gavi + Paca cast. Session 09 then completed the product-surface integration: Home now exposes Belajar / World / Bermain with the approved five-character ensemble, while Bermain catalog/preflight/completion use Gavi + Paca through the same shared presentation runtime.
...
Canonical closures: `MAINLAGI_WORLD_CHARACTER_INTEGRATION_SESSION08_2026-09-25.md` and `MAINLAGI_HOME_BERMAIN_CHARACTER_INTEGRATION_SESSION09_2026-09-25.md`. Canonical handoff remains `MAINLAGI_CHARACTER_WORLD_SAFE_CHECKPOINT_2026-09-25.md`.
...
World migration: complete through Session 08; Home/Bermain shell integration: complete through Session 09
```

All 35 character variants are exact SHA-bound, owned/provenance-approved production SVGs under `public/artwork/characters/`. Belajar now resolves its canonical subject pair through the shared runtime: English = Naya+Zia, Math = Gian+Paca, and Bahasa/Iqro/Huruf/Logic/Science = Gavi+Paca. Color/Drawing retain the Gavi+Paca policy but suppress the decorative foreground while the creative workspace is active. World is migrated through Session 08 and Home/Bermain product-surface integration is complete through Session 09. The next authorized sequence item is Session 10 semantic/activity illustration registry SVG-aware production work.
...
This authorization is now implemented through Session 09: all 35 SVGs are production-approved/runtime-addressable; Belajar uses the shared resolver + CharacterLayer with a clean 320/390/430/768/1280 representative matrix; Petualangan Uang uses the same shared runtime for its authored Gavi + Paca cast; and Home/Bermain integration is merged/live verified through PR #340.
