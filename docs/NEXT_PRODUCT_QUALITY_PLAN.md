...
16. **NOW / TWO PARALLEL SAFE TRACKS:** (a) keep car 499718 / towel 288034 / raincoat 212019 fail-closed behind the project-owner redistribution decision gate; do not repeat the same source hunt as production clearance; safe exits remain authoritative exact open-redistribution evidence, express written standalone-distribution permission, or a new explicit project-owner visual replacement decision. Previously rejected 118911 / 297932 / 490914 remain historical only and must not be silently adopted. (b) independently advance the other 14/17 clear assets through the new **internal production-readiness preflight** only: exact normalized source scope, deterministic 512×512 alpha WebP, production-basename/SHA/technical checks, hard refusal of `public/`, and zero registry/runtime mutation. This preflight is not production approval;
