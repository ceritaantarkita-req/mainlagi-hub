  ...
  try {
...

    {
      ...
      const openFromCatalog = async (activityId) => {
        ...
        await page.locator(`[data-activity-id="${activityId}"] a`).waitFor();
        await page.waitForTimeout(120);
        const documentMarker = `qa-client-nav-${activityId}`;
        await page.evaluate((marker) => { window.__mainlagiAudioQaDocumentMarker = marker; }, documentMarker);
        await page.locator(`[data-activity-id="${activityId}"] a`).click();
        await page.waitForURL(new RegExp(`/child/demo-gian/activity/${activityId}import assert from "node:assert/strict";
import { spawn } from "node:child_process";
import { mkdirSync, rmSync, writeFileSync } from "node:fs";
import path from "node:path";
import process from "node:process";
  ...
  try {
...

    {
      ...
      const page = await context.newPage();
        ...
        assert.equal(
          await page.evaluate(() => window.__mainlagiAudioQaDocumentMarker),
          documentMarker,
          "activity entry audio QA requires hydrated client-side navigation"
        );
        await page.waitForFunction(
        ...
        return page.evaluate((expectedId) => (
          window.__mainlagiAudioEntrySamples.find((sample) => sample.activityId === expectedId)
        ), activityId);
      };

      const genericSample = await openFromCatalog("english-find-blue");
      assert.equal(genericSample.activityId, "english-find-blue");
      assert.equal(genericSample.lang, "en-US", "English activity navigation must pre-warm English rather than default Indonesian");
      assert.equal(genericSample.source, "navigation", "activity-link gesture must be correlated with entry narration");
