...
const viewports = [
  { width: 320, height: 720, completionMode: "touch" },
  { width: 390, height: 844, completionMode: "pointer" },
  { width: 768, height: 1024, completionMode: "touch" }
];
let server = null;
let serverLog = "";
...
async function chooseCorrect(page, mode) {
  ...
  assert(box, "correct English cloze choice must have touchable bounds");
  await page.touchscreen.tap(box.x + box.width / 2, box.y + box.height / 2);
}

async function inspect(viewport) {
  const hasTouch = viewport.completionMode === "touch";
  ...
  try {
    ...
    assert.equal(
      ...
      route,
      `progression guard must accept legitimate English prior-stage readiness at ${viewport.width}`
    );

    await page.getByRole("heading", { name: "Complete the sentence", exact: true }).waitFor({ state: "visible" });
    assert.equal(
      await page.locator("[data-cloze-sentence-choice]").getAttribute("data-cloze-locale"),
      "en-US",
      "English cloze must expose reviewed English locale"
    );
    ...
    assert.equal(await sentence.count(), 1, "English canonical sentence renders as one cloze surface");
    assert.equal(await slot.count(), 1, "English cloze renders one visible blank");
    ...
    assert.equal(await choices.count(), 3, "English cloze keeps three canonical choices");
    assert.deepEqual(await choices.allTextContents(), ["SLEEPS", "BOOK", "YELLOW"], "English choice order and labels remain canonical");
    assert.equal(await completed(page), false, "idle English cloze state cannot complete activity");
    ...
    await status.filter({ hasText: "Read the whole sentence" }).waitFor({ state: "visible", timeout: 2000 });
    ...
    assert.equal((await slot.textContent())?.trim(), wrongChoice, "wrong keyboard selection binds visibly into English blank");
    assert.equal(await slot.getAttribute("data-slot-state"), "try", "English slot exposes retry state without completing");
    assert.equal(await completed(page), false, "wrong English cloze choice cannot complete activity");
    ...
    await chooseCorrect(page, viewport.completionMode);
    ...
    assert.equal(await completed(page), true, `correct English cloze ${viewport.completionMode} selection completes canonical activity`);
    await assertFullyVisible(status, viewportHeight, `English success feedback at ${viewport.width}`);
    const nextLink = page.getByRole("link", { name: "Choose another game", exact: true });
    ...
    assert.equal(state.assessed, true, "English cloze remains assessed");
    ...
    assert.deepEqual(pageErrors, [], `English cloze page errors at ${viewport.width}: ${pageErrors.join(" | ")}`);
    assert.deepEqual(consoleErrors, [], `English cloze console errors at ${viewport.width}: ${consoleErrors.join(" | ")}`);
    await context.close();
  } finally {
    await browser.close();
  }
...
async function main() {
  ...
  console.log(
    `English cloze reuse browser QA passed ${viewports.length} viewports with nine screenshots, English locale/copy, keyboard retry, pointer + actual-touch completion, touch targets and assessed evidence checks.`
  );
}
