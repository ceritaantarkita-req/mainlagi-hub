...
assert.equal(DRAWING_GUIDE_IDS.length,100,"final WS-07 wave must provide an explicit scaffold for all 100 Drawing activities");
...
const finalIds=new Set([
  "drawing-invent-flying-car","drawing-invent-helper-robot","drawing-invent-fantasy-house","drawing-invent-animal-mix","drawing-invent-playground",
  "drawing-character-hat","drawing-character-job","drawing-character-emotion","drawing-character-pet","drawing-character-costume",
  "drawing-map-bedroom","drawing-map-playground","drawing-map-treasure","drawing-map-island","drawing-map-space-base",
  "drawing-design-badge","drawing-design-flag","drawing-design-book-cover","drawing-design-sign","drawing-design-poster",
  "drawing-capstone-favorite-place","drawing-capstone-new-creature","drawing-capstone-machine","drawing-capstone-mini-story","drawing-capstone-free-studio"
]);
assert.equal(waveAIds.size,25,"Wave A scope must remain 25 activities");
assert.equal(waveBIds.size,25,"Wave B scope must remain 25 activities");
assert.equal(finalIds.size,25,"final sparse-scaffold scope must remain 25 activities");

for(const id of [...waveAIds,...waveBIds,...finalIds]){
...
assert.equal(remainingYoungWithoutGuide.length,0,"final WS-07 wave must resolve Q106-equivalent missing scaffolds without weakening the rule");
...
try{
  ...
  for(const id of DRAWING_GUIDE_IDS){
    ...
    assert.equal(metrics.length,guide.paths.length,`all scaffold paths must parse: ${id}`);
  ...
  console.log(`Drawing scaffold regression PASS: ${DRAWING_GUIDE_IDS.length}/100 activities have explicit functional guides; Q106-equivalent remainder ${remainingYoungWithoutGuide.length}; ${finalIds.size} final sparse-scaffold preview artifacts exported.`);
}finally{
  await browser.close();
}
