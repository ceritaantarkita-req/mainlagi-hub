...
try {
  ...
  for(const viewport of [{width:1440,height:1000},{width:390,height:844},{width:1505,height:1045}]){
    ...
    assert.equal(await page.locator("[data-activity-id]").count(),100,"all 100 cards visible without stage navigation");
    assert.equal(await page.locator('a[href^="/child/demo-gian/stage/"]').count(),0);
    await page.locator('a[href^="/child/demo-gian/activity/"]').first().waitFor();
    await page.getByRole("button",{name:"Lingkaran besar, belum terbuka",exact:true}).click();
    await page.getByRole("dialog",{name:"Pilih gambar bertanda main"}).waitFor();
    ...
    result.checks.push("single-level child navigation "+viewport.width);
  ...
  assert.equal(DRAWING_GUIDE_IDS.length,25,"25 explicit foundational drawing scaffolds");
  assert.equal(new Set(DRAWING_GUIDE_IDS.map(id=>JSON.stringify(drawingGuide(id)))).size,25,"scaffolds are not identical");
  ...
  result.checks.push("25 real drawing scaffolds, direct routes, matching thumbnails, no synthetic completion");
