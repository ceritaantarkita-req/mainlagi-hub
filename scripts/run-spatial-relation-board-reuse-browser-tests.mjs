...
const port=Number(process.env.MAINLAGI_SPATIAL_RELATION_REUSE_QA_PORT??4042);
const baseUrl=`http://${host}:${port}`;
const route="/child/demo-gian/activity/math-spatial-near";
const activityId="math-spatial-near";
const correctLabel="Pilih jawaban pintu";
...
async function waitForServer(){
  ...
  throw new Error(`Math spatial reuse QA server did not become ready.\n${serverLog.slice(-4000)}`);
}
...
async function seedPrerequisiteReadiness(context){
  await context.addInitScript(()=>{
    ...
    const attempts=seeds.map(([seedActivityId,skillId,stageId,runtime],index)=>{
      const attemptId=`qa-math-spatial-reuse-prereq-${index}`;
      const completedAt=`2026-09-19T02:${String(index).padStart(2,"0")}:00.000Z`;
      return{
        ...
        metadata:{source:"math-spatial-reuse-browser-prerequisite"},
...
async function waitForScene(page){
  ...
  await page.waitForFunction(()=>{
    ...
    return scene?.getAttribute("data-spatial-relation-board-ready")==="true"
      && scene.querySelectorAll("[data-spatial-relation-choice]").length===3
      && scene.querySelectorAll("[data-spatial-relation-visual]").length===1;
...
async function keyboardWrongChoice(page){
  for(let step=0;step<96;step+=1){
    ...
    const focused=await page.evaluate(()=>({
      choice:Boolean(document.activeElement?.hasAttribute?.("data-spatial-relation-choice")),
  ...
  throw new Error("Keyboard navigation did not reach a wrong Math spatial choice");
}
...
async function inspect(viewport){
  ...
  try{
    ...
    assert(response&&response.status()<400,`Math spatial reuse bad HTTP at ${viewport.width}`);
    ...
    assert.equal(await page.getByText("Paca berdiri dekat pintu dan jauh dari pohon. Apa yang lebih dekat ke Paca?").count(),1,"canonical Math prompt remains visible");
    const board=page.locator("[data-spatial-relation-visual]");
    assert.equal(await board.getAttribute("data-relation-kind"),"near");
    assert.equal(await board.getAttribute("data-relation-mode"),"proximity");
    assert.equal(await board.getByText("Paca",{exact:true}).count(),1,"focus object remains visible");
    assert.equal(await board.getByText("🚪 Pintu",{exact:true}).count(),1,"near target remains visible");
    assert.equal(await board.getByText("🌳 Pohon",{exact:true}).count(),1,"far target remains visible");

    const choices=page.locator("[data-spatial-relation-choice]");
    assert.equal(await choices.count(),3);
    assert.deepEqual(await choices.allTextContents(),["pintu","pohon","sama"],"Math choice order remains canonical");
    assert.equal(await completed(page),false);

    const viewportWidth=await page.evaluate(()=>document.documentElement.clientWidth);
    const viewportHeight=await page.evaluate(()=>window.innerHeight);
    const scrollWidth=await page.evaluate(()=>Math.max(document.documentElement.scrollWidth,document.body.scrollWidth));
    assert(scrollWidth<=viewportWidth+1,`Math spatial board overflows horizontally at ${viewport.width}`);
    for(const box of await choices.evaluateAll(items=>items.map(node=>{const rect=node.getBoundingClientRect();return{width:rect.width,height:rect.height,left:rect.left,right:rect.right};}))){
      assert(box.width>=44&&box.height>=44,"Math spatial choices keep minimum touch target");
      assert(box.left>=-1&&box.right<=viewportWidth+1,"Math spatial choices remain inside viewport");
    }

    const status=page.getByRole("status");
    await assertFullyVisible(status,viewportHeight,`idle Math spatial feedback at ${viewport.width}`);
    mkdirSync(screenshotDir,{recursive:true});
    await page.screenshot({path:path.join(screenshotDir,`${viewport.width}-spatial-relation-math-near-idle.png`),fullPage:false});
    ...
    assert.equal(await completed(page),false,"wrong Math spatial answer cannot complete");
    await assertFullyVisible(status,viewportHeight,`retry Math spatial feedback at ${viewport.width}`);
    await page.screenshot({path:path.join(screenshotDir,`${viewport.width}-spatial-relation-math-near-try.png`),fullPage:false});
    ...
    assert(state,"Math spatial board records attempt evidence");
    ...
    assert.equal(state.metadata?.mode,"proximity");
    assert.equal(state.metadata?.relationOrTurn,"near");
    assert.equal(state.metadata?.selectedChoice,"pintu");
    assert.equal(state.correctCount,1);
    assert.equal(state.incorrectCount,1);
    assert.equal(state.retryCount,1);
    assert.equal(state.accuracy,0.5);

    await page.screenshot({path:path.join(screenshotDir,`${viewport.width}-spatial-relation-math-near-success.png`),fullPage:false});
